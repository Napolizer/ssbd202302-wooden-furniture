...
public class MOK6IT {
  ...
  class Negative {
    ...
    void failsToRemoveAccessLevelWithoutAuthorizationToken() {
      given()
              .when()
              .delete("/account/id/" + InitData.retrieveAccountId("accounttoeditals") + "/accessLevel/Administrator")
              .then()
              .statusCode(401);
    }
    ...
    void failsToRemoveAccessLevelWhenAccessLevelIsInvalid() {
      given()
              .header("Authorization", "Bearer " + InitData.retrieveAdminToken())
              .when()
              .delete("/account/id/" + InitData.retrieveAccountId("accounttoeditals") + "/accessLevel/NotValidAccessLevel")
              .then()
              .statusCode(400)
    ...
    void failsToRemoveAccessLevelWhenThereIsOnlyOneAccessLevelAssigned() {
      given()
              .header("Authorization", "Bearer " + InitData.retrieveAdminToken())
              .when()
              .delete("/account/id/" + InitData.retrieveAccountId("accounttoeditals") + "/accessLevel/Employee")
              .then()
              .statusCode(400)
              .body("message", equalTo(MessageUtil.MessageKey.ACCOUNT_REMOVE_ACCESS_LEVEL));
    }
