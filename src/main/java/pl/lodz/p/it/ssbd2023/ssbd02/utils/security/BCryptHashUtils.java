...
public final class BCryptHashUtils {
    ...
    private static final SecureRandom secureRandom = new SecureRandom(secretKey.getBytes(StandardCharsets.UTF_8));
    ...
    public static String hashVersion(Long version) {
        return BCrypt.with(secureRandom).hashToString(BCRYPT_COST_FACTOR, Long.toString(version).toCharArray());
    }

    public static boolean verifyVersion(Long version, String hash) {
        return BCrypt.verifyer().verify(Long.toString(version).toCharArray(), hash.toCharArray()).verified;
    }
